...
local function HandleLockpickDurability(playerSource, item)
    local metadata = item.metadata
    local ostime = os.time()
    local degrade = (metadata.degrade * 60)

    -- Different degrade % depending on lockpick type
    local degradePercentage = item.name == 'advancedlockpick' and 10 or 20
    local removeSeconds = (degrade * degradePercentage) / 100

    metadata.durability = math.ceil(metadata.durability - removeSeconds)

    local percentage = ((metadata.durability - ostime) * 100) / degrade

    if percentage <= 0 then
        metadata.durability = 0
        exports.ox_inventory:RemoveItem(playerSource, item.name, 1, nil, item.slot)
        exports.qbx_core:Notify(playerSource, "Your lockpick has broken.", 'error')
        return false
    else
        -- update metadata back on the item slot
        exports.ox_inventory:SetMetadata(playerSource, item.slot, metadata)
        return true
    end
end
...
AddEventHandler('lockpicks:UseLockpick', function(playerSource, isAdvanced)
    ...
    local levels =  exports['stressy-levels']GetAllXP(playerSource)
    if levels.criminal.level < 4 and levels.electric.level < 2 then
        lib.notify({ type = 'error', description = "You are not ready to this yet!" })
        return
    end
    ...
    local ok = HandleLockpickDurability(playerSource,item)
    if not ok then return end -- stop if it broke
...
-- Reward snippet with chance-based cash and dirty money
local function giveCashReward(src)
    local roll = math.random(1, 100) -- roll between 1 and 100

    if roll <= 40 then
        -- 5% chance: dirty money
        local dirtyAmount = math.random(300, 1000)
        exports.ox_inventory:AddItem(src, 'dirty_money', dirtyAmount)

        TriggerClientEvent('ox_lib:notify', src, {
            title       = 'Lucky Find',
            description = ('You found $%s in dirty money!'):format(dirtyAmount),
            type        = 'success'
        })

    elseif roll <= 5 then
        -- 55% chance: cash
        local cashAmount = math.random(500, 1500)
        exports.ox_inventory:AddItem(src, 'money', cashAmount)

        TriggerClientEvent('ox_lib:notify', src, {
            title       = 'Reward',
            description = ('You received $%s cash.'):format(cashAmount),
            type        = 'inform'
        })
...
RegisterNetEvent('qbx_houserobbery:server:lootFinished', function(houseIndex, lootIndex)
    ...
    giveCashReward(player.PlayerData.source)
