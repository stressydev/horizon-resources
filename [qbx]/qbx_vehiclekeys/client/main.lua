...
local function toggleEngine(vehicle)
    ...
    if cache.vehicle then
        if engineOn then
            engineForcedOff = true    -- player turned engine OFF
        else
            engineForcedOff = false   -- player turned engine ON
        end
...
CreateThread(function()
    while true do
        ...
        if cache.vehicle and cache.seat == -1 and engineForcedOff then
            local veh = cache.vehicle

            -- Stop GTA auto-start
            DisableControlAction(0, 71, true) -- W
            DisableControlAction(0, 72, true) -- S (optional)

            -- Keep engine off because PLAYER turned it off
