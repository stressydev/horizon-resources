...
local function breakLockpick(isAdvancedLockedpick, vehicle)
    ...
    local lockpicks = exports.ox_inventory:Search('slots', lockpickItem)
    for k,v in pairs(lockpicks) do
        local metadata = v.metadata
        local ostime = lib.callback.await('qbx_vehiclekeys:getOsTime', false)
        local degrade = (metadata.degrade * 60)

        local degradePercentage = 20 -- 20% per use
        ...
        if percentage <= 0 then
            ...
            lib.notify({ title = 'Item', description = "Your cracking tool has broken.", type = "error", })
            TriggerServerEvent('qbx_vehiclekeys:removeBrokenItem', GetCurrentResourceName(), v.name, v.slot)
            break
        end

        lib.callback.await('qbx_vehiclekeys:setItemMetadata', false, v.slot, metadata)
        break
    end
end
