...
lib.addCommand('ooc', {
    ...
    for _, v in pairs(players) do
        if IsPlayerAceAllowed(v --[[@as string]], 'admin') then
            exports.chat:addMessage(v --[[@as Source]], {
                color = { 155, 48, 255 }, -- purple accent for OOC
                multiline = true,
            ...
            logger.log({
                ...
                message = ('**%s** (CitizenID: %s | ID: %s) **Message:** %s')
                    :format(GetPlayerName(source), player.PlayerData.citizenid, source, message)
            })
        end
    end
end)
