...
exports('registerCustomDoor', function(id, doorTable, name)
    assert(type(id) == 'number', 'Door ID must be a number')
    assert(type(doorTable) == 'table', 'doorTable must be a table')
    assert(type(name) == 'string', 'Door name must be a string')

    local door = createDoor(id, doorTable, name)
    print(('[ox_doorlock] Registered local door "%s" (id %s)'):format(name, id))
    return door
end)
